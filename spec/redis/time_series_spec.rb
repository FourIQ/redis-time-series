...
RSpec.describe Redis::TimeSeries do
  ...
  describe "TS.ALTER" do
    context "when altering the retention period" do
      specify do
        expect { ts.retention = 1234 }.to issue_command("TS.ALTER #{key} RETENTION 1234")
      end
    end

    context "when altering the labels" do
      specify do
  ...
  describe "TS.DEL" do
    ...
    context "when the series does not exist" do
      ...
      it "still raises for errors other than a missing key" do
        ...
        redis.with { |conn| conn.del(missing_ts.key) }
      end
    end
  end
  ...
  describe "TS.RANGE" do
    ...
    context "with a timestamp filter" do
      specify do
        expect { ts.range(from..to, filter_by_ts: [10, 20, 30]) }.to issue_command(\
          "TS.RANGE #{key} #{msec(from)} #{msec(to)} FILTER_BY_TS 10 20 30")
      end
    end

    context "with a value filter" do
      specify do
        expect { ts.range(from..to, filter_by_val: [10, 30]) }.to issue_command(\
          "TS.RANGE #{key} #{msec(from)} #{msec(to)} FILTER_BY_VALUE 10 30")
      end
    end
    ...
    context "with an aggregation" do
      specify do
        expect { ts.range(from..to, aggregation: [:avg, 60_000]) }.to issue_command(\
          "TS.RANGE #{key} #{msec(from)} #{msec(to)} ALIGN start AGGREGATION avg 60000 empty")
      end

      context "with a timestamp filter" do
        specify do
          expect { ts.range(from..to, filter_by_ts: [10, 20, 30], aggregation: [:avg, 60_000]) }.to issue_command(\
            "TS.RANGE #{key} #{msec(from)} #{msec(to)} FILTER_BY_TS 10 20 30 ALIGN start AGGREGATION avg 60000 empty")
        end
      end

      context "with a value filter" do
        specify do
          expect { ts.range(from..to, filter_by_val: [10, 30], aggregation: [:avg, 60_000]) }.to issue_command(\
            "TS.RANGE #{key} #{msec(from)} #{msec(to)} FILTER_BY_VALUE 10 30 ALIGN start AGGREGATION avg 60000 empty")
        end
      end
    ...
    context "with an aggregation" do
      specify do
        expect { ts.revrange(from..to, aggregation: [:avg, 60_000]) }.to issue_command(\
          "TS.REVRANGE #{key} #{msec(from)} #{msec(to)} ALIGN start AGGREGATION avg 60000 empty")
      end
