...
## Unreleased
* A `Time` goes over the wire as its milliseconds, floored, instead of whole seconds (`to_i * 1000`). Every bound and timestamp passes through `Client.wire`, so this reaches every read: an `end_of_day` bound now reaches 23:59:59.999 instead of stopping at 23:59:59.000 and dropping the last second of the day, and `TS.ADD` keeps the milliseconds of a `Time` or `TimeWithZone`.
