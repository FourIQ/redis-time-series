...
* Run boundaries are millisecond-precise, through `filter_by_range` as well. Two layers dropped the sub-second part independently: `start_time`/`end_time` divided a millisecond bound by an Integer `1000` and so floored a run boundary to the whole second, and a clipped bound handed on as a `Time` is serialised `to_i * 1000`. Sub-ranges now go to Redis as milliseconds.
* `filter_by_range` restores `align` as well as the window, so a `RangeCmd` is left as it was found.
* Daily run bounds go to Redis as milliseconds. Handed on as a `Time` they were serialised `to_i * 1000`, so a window starting mid-second closed each run at `.499` and reopened the next at `.000` — the samples in between were returned by both commands and counted twice.
